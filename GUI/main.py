...
class MainWindow(Tk):
    ...
    def GUI(self):
        ...
        Button(window, width=12, text="最佳行", command=self.Bestline_Button).grid(row=1, column=4)
        Button(window, width=12, text="最大利润", command=self.Maxmoney_Button).grid(row=2, column=4)
        Button(window, width=12, text="重置", command=self.Reset_Button).grid(row=3, column=4)

    def Bestline_Button(self):
        x = []
        y = []
        for i in  range(9):
            num = self.pos[i].get()
